...
#NSX Edge Related CloudBuilder Config
    ...
    if($($userOptions.mgmtNetVlan) -ne 0) {
		...
		## JS Insert 8-11-22 add HA isolation address to supress warning in vCenter/Disable
		$replaceNet +="echo Address=$global:vsanNetGateway/$global:vsanNetCIDR`n"
		## End JS Insert 8-11-22
...
{ 
	...
	### JS Insert Add parse for VSAN gateway to set HA Isolation address  JS 8-11-22
    $global:txtVSANNet = $($bringupObject.networkSpecs| where { $_.networkType -eq "VSAN" } | Select subnet).subnet
	$global:txtVSANGateway = $($bringupObject.networkSpecs| where { $_.networkType -eq "VSAN" } | Select gateway).gateway
	$global:vsanNetGateway=$global:txtVSANGateway
    $global:vsanNetCIDR=$($global:txtVSANNet).Substring(($($global:txtVSANNet).IndexOf("/")+1),($($global:txtVSANNet).Length - ($($global:txtVSANNet).IndexOf("/") +1)))
    ### End JS insert JS 8-11-22
                    ...
                    if ($btnSubmit.Text -eq "Construct!")
                        {
							...
							### JS Insert 8-11-22
							#logger "VSAN Gateway $global:txtVSANGateway"
							#logger "VSAN Network $global:txtVSANNet"
							#"vsanNetGateway"=$global:txtVSANGateway
                            #"vsanNetCIDR"=$($global:txtVSANNet).Substring(($($global:txtVSANNet).IndexOf("/")+1),($($global:txtVSANNet).Length - ($($global:txtVSANNet).IndexOf("/") +1)))
                            ### End JS Insert
...
$startupHostCode = {
	...
	## JS Insert 8-11-22 update ISO Path to use vmPrefix
...
## JS Insert 8-11-22 change ISO location to use vmPrefix
$ds = Get-Datastore $userOptions.ds
logger "Uploading Custom vSphere ISO to the \ISO-$vmPrefix directory of $($ds)"
