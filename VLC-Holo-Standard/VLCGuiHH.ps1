###################################################################
# VCF HH Version - Lab Constructor beta v4.4.1 8/29/2022
# Created by: bsier@vmware.com;hjohnson@vmware.com;ktebear@vmware.com
# QA: stephenst@vmware.com;acarnie@vmware.com;jsenika@vmware.com
#
...
#region CloudBuilder Import
    ...
    if([bool]$userOptions.useCBIso) {
        ...
        If (Test-Path -Path $copyEsxiPath) {
            ...
            $vSphereISOLoc = $(Get-ChildItem -Path "$scriptDir\$tempDir\*.iso" | Select FullName).FullName        
        }
    } else {
...
Copy-DatastoreItem -Item ("$scriptDir\$tempDir\$currIso")  -Destination ("vsands:\ISO\" + $vmPrefix + "VLC_vSphere.iso") -Force
